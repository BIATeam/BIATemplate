...
namespace TheBIADevCompany.BIATemplate.Domain.Bia.Base.Mappers
{
    using System;
        ...
        where TTeam : class, IEntity<int>, IEntityTeam, new()
    {
        ...
        public BaseTeamMapper(IPrincipal principal)
            : base(principal)
        {
            Debug.Assert(typeof(IEntityTeam).IsAssignableFrom(typeof(TTeam)), typeof(TTeam) + " should im�plement IEntityTeam");
            Debug.Assert(typeof(IDtoTeam).IsAssignableFrom(typeof(TTeamDto)), typeof(TTeamDto) + " should im�plement IDtoTeam");
